(async () => {
  ...
  if (window.__rofanAiExporterRunning) {
    alert("이미 실행 중입니다.");
    return;
  }

  window.__rofanAiExporterRunning = true;
  ...
  try {
    ...
    const fstMsg = bot.first_message || "";
    const chatIndex = Number(props?.oriChatData?.chat_count) || 100;
    const characterName = props?.oriChatData?.char || bot.char || "character";
    ...
    window.__rofanAiExporterRunning = false;
  }
})();      if (closeIndex === -1) return null;
